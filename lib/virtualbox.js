...
function list(arg, callback){
  logging.info('Listing "%s"', arg);
  if((arg !== 'vms') && (arg !== 'runningvms')) {
    _err = new Error('The "' + arg + '" list command is not supported - use vms or runningvms');
    callback({}, _err);
  }
  vboxmanage('list "'+arg+'"', function(error, stdout){
    callback(parse_listdata(stdout), error);
  });
}
  ...
  if (_raw.length > 0) {
    for(var _i=0; _i < _raw.length; _i += 1){
      ...
      if(arrMatches.length === 3) {
        _data[arrMatches[2].toString()] = arrMatches[1].toString();
      }
    }
  }
  return _data;
